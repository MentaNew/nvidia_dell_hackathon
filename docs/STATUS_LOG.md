...
| 09-12 15:13 | Separate "Live GB10" page (`/live.html`, `GET /api/telemetry`): GPU (nvidia-smi), system (procfs), per-provider inference latency p50/p95, ingestion throughput, log counts, network, store | Requested performance page; main UI and SQLite untouched | Hardware readings only when the server detects an NVIDIA GB10 (or `RESCUEBASE_ASSUME_GB10=1`), else "Unavailable — awaiting GB10 telemetry"; laptop numbers never shown as GB10 data; `test_telemetry` covers the parser (incl. `[N/A]`) and the unavailable path | tested with stubs (laptop); **actual GB10 metrics NOT TESTED** |
